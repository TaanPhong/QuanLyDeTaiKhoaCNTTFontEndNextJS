...
export default function TableClass(props) {
    const { change, setChange, setClasses, setShow, setUpdate, subjectCode, search, isLoading, setIsLoading } = props;
    ...
    React.useEffect(() => {
        if (subjectCode == 0) {
            axios.get("http://localhost:8081/classes")
                .then(res => {
                    const dataClasses = res.data.map((item, index) => {
                        return {
                            stt: index + 1,
                            ...item,
                        }
            ...
            axios.get("http://localhost:8081/class/" + subjectCode)
                .then(res => {
                    const dataClasses = res.data.map((item, index) => {
                        return {
                            stt: index + 1,
                ...
                .catch(err => {
                    toast.error("Hệ thống xảy ra lỗi!")
                    console.log("Lỗi lấy lớp theo bộ môn", err)
                })
        }
    }, [change, subjectCode])
        ...
        () => {
            if (search) {
                return stableSort(rows, getComparator(order, orderBy)).filter(
                    item => item.className.toLowerCase().indexOf(search.toLowerCase().trim()) !== -1)
                    .slice(
                        page * rowsPerPage,
    ...
    const handleDelete = (row) => {
        if (row.studentDTOs) {
            toast.info("Lớp này đã có sinh viên không thể xóa")
        }
        else {
            if (window.confirm("Bạn thật sự muốn xóa lớp này?")) {
                axios.delete("http://localhost:8081/class/" + row.classCode)
                    .then(res => {
                            ...
                            {visibleRows.map((row, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell align="left" sx={{ width: '250px' }}>{row.className}</TableCell>
