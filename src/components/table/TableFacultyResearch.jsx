...
export default function EnhancedTable(props) {
    const { selected, setSelected, change, setChange, setResearch, setShowModalsDetail,
        schoolYearId, search, isLoading, setIsLoading, status } = props;
    ...
    React.useEffect(() => {
        const check = rows.find((item) => {
            return item.researchStatusName === "Chờ duyệt" && item.schoolYearName === context.schoolYearMax.schoolYearName && checkDate;
        })
        if (check) {
    ...
    const displayCheckBoxInRow = (row, isItemSelected, labelId) => {
        if (row.researchStatusName === "Chờ duyệt" && row.schoolYearName === context.schoolYearMax.schoolYearName && checkDate) {
            return (
    ...
    React.useEffect(() => {
        if (schoolYearId == 0) {
            if (!status) {
                ...
                axios.get("http://localhost:8081/research/faculty/" + status)
                    .then(res => {
                        let getRow = res.data.map((item, index) => {
                            return {
                                id: index + 1,
                    ...
                    .catch(err => {
                        toast.error("Quá trình lấy dữ liệu danh sách để tài theo niên khóa xảy ra lỗi")
                        console.log("Thư ký khoa - để tài", err)
                    })
            }
            ...
            if (!status) {
                ...
                axios.get("http://localhost:8081/research/faculty/" + schoolYearId + "/" + status)
                    .then(res => {
                        let getRow = res.data.map((item, index) => {
                            return {
                                id: index + 1,
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = [];
            rows.forEach(item => {
                if (item.researchStatusName === 'Chờ duyệt'
