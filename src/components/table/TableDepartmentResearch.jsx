...
export default function EnhancedTable(props) {
    const { selected, setSelected, change, setResearch, setShowModalsDetail, search, schoolYearId,
        setShowNote, isLoading, setIsLoading, status } = props;
    ...
    React.useEffect(() => {
        const check = rows.find((item) => {
            return item.researchStatusName === "Đang duyệt" || item.researchStatusName === "Đã chỉnh sửa"
    ...
    const displayCheckBoxInRow = (row, isItemSelected, labelId) => {
        if (row.researchStatusName === "Đang duyệt" || row.researchStatusName === "Đã chỉnh sửa"
    ...
    React.useEffect(() => {
        if (schoolYearId == 0) {
            axios.get('http://localhost:8081/research/' + context.account?.subjectCode)
                .then(res => {
            ...
            axios.get("http://localhost:8081/research/department/" + schoolYearId + "/" + context.account?.subjectCode)
                .then(res => {
                    let getRow = res.data.map((item, index) => {
                        return {
                            id: index + 1,
                ...
                .catch(err => {
                    toast.error("Quá trình lấy danh sách dữ liệu đề tài xảy ra lỗi")
                    console.log("Lỗi lấy danh sách đề tài theo trưởng bộ môn - niên khóa", err)
                })
        }
    }, [change, schoolYearId])
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelected = [];
            rows.forEach(item => {
                if (item.researchStatusName === 'Đang duyệt' || item.researchStatusName === "Đã chỉnh sửa"
                    && item.schoolYearName === context.schoolYearMax.schoolYearName && checkDate
                ) {
    ...
    const checkUpdate = (row) => {
        if (row.researchStatusName === "Đang duyệt" || item.researchStatusName === "Đã chỉnh sửa"
            && row.schoolYearName === context.schoolYearMax.schoolYearName && checkDate) {
            setResearch(row)
            setShowNote(true);
        }
        else {
            toast.warning("Chỉ có thể thực hiện yêu cầu chỉnh sửa trên đề tài có trạng thái là đang duyệt")
        }
    }
                            ...
                            {visibleRows.map((row, index) => {
                                ...
                                return (
                                    <TableRow
                                        ...
                                        sx={{ background: row.researchStatusName === "Đã chỉnh sửa" ? '#35A6E8' : null }}
                                    >
                                        {
